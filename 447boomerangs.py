...
class Solution(object):
    def numberOfBoomerangs(self, points):
        ...
        # euculidian squared distance in 2D
        sqrDist = lambda a,b: (a[0]-b[0])*(a[0]-b[0]) + (a[1]-b[1])*(a[1]-b[1]) 
        # pack every pair of points together with the pre-calculated squared distance
        distPairs = [ (sqrDist(i,j), i, j) for i in points2D for j in points2D if i > j ]
        # a rather generally useful reduceByKey function where the key is the 0th elemet here
        reduceByKey = lambda x : { b[0]: map(lambda c:(c[1:]),b[1]) for b in itertools.groupby( sorted(x,key=lambda a:a[0]), key=lambda a:a[0]) }
        # make a map of every distance to the array of pairs of points
        groupped = reduceByKey(distPairs)
        # for each key (distance) flatten the array of pairs
        flattened = { d : [ point  for pair in a  for point in pair ] for d, a in groupped.iteritems() }
        # supply every point with a count and count the entries
        counts = { d : { p:len(list(e)) for p,e in itertools.groupby( sorted(a)) } for d,a in flattened.iteritems() }
        # strip points leaving only counts of all pairs
